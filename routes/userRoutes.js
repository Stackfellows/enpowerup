...
router.post("/register", async (req, res) => {
  try {
    ...
    let upliner = null;
    if (uplineId && mongoose.Types.ObjectId.isValid(uplineId)) {
      upliner = await User.findById(uplineId);
      if (!upliner) {
        console.log("❌ Upliner not found in DB with provided ID:", uplineId);
      } else {
        console.log(
          "✅ Upliner found:",
          upliner.name,
    ...
    if (selectedPackage) {
      ...
      // If no package selected, new user gets 0 points and 0 discount initially
      await distributePointsAndDiscountsUpTheChain(
        newUser._id,
        0, // Purchaser points
        0, // Upliner points
        0, // Team Leader points
